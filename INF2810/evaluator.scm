...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((and? exp) (eval-and (and-checks exp) env))
        ((or? exp) (eval-or (or-checks exp) env))
...
(define (eval-and exp env)
  (cond ((false? (mc-eval (car exp) env)) #f)
        ...
        ((true? (mc-eval (car exp) env)) (eval-and (cdr exp) env))))

(define (eval-or exp env)
  (cond ((true? (mc-eval (car exp) env)) #t)
        ((null? (cdr exp)) (car exp))
        ((false? (mc-eval (car exp) env)) (eval-or (cdr exp) env))))
...
(define (eval-while exp env)
  (let ((pred (car exp))
        (body (cdr exp)))
    (cond ((mc-eval pred env)
           (eval-iter body env)       
           (eval-while exp env)))))

(define (eval-iter exp env)
  (cond ((not (null? exp))
         (mc-eval (car exp) env)
         (eval-iter (cdr exp) env))))
